    ...
    for (int control_update_step = 0; control_update_step < max_control_updates; control_update_step++) {
        ...
        for (uint32_t i = 0; i < solve_count; i++) {
            ...
            if (!shifted_vec[i] && time_since_timestep_vec[i] > shift_threshold) {
                ...
                just_shift<T>(state_size, control_size, knot_points, d_xu + i * traj_len);
                just_shift<T>(6, 0, knot_points, d_eePos_goal + i * 6 * knot_points);
                just_shift<T>(state_size, 0, knot_points, d_lambda + i * state_size * knot_points);
