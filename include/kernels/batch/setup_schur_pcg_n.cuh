...
template <typename T>
__device__
void complete_SS_Pinv_blockrow_batched(uint32_t state_size, uint32_t knot_points, T *d_S, T *d_Pinv, T *d_gamma, T *s_temp, unsigned blockrow){
...
template <typename T>
__device__
void form_S_gamma_and_jacobi_Pinv_blockrow_batched(uint32_t state_size, uint32_t control_size, uint32_t knot_points, T *d_G, T *d_C, T *d_g, T *d_c, T *d_S, T *d_Pinv, T *d_gamma, T rho, T *s_temp, unsigned blockrow){
...
template <typename T>
__global__ void form_schur_system_kernel(
    uint32_t state_size,
    ...
    form_S_gamma_and_jacobi_Pinv_blockrow_batched<T>(
    ...
    complete_SS_Pinv_blockrow_batched<T>(
        state_size, knot_points,
        traj_S, traj_Pinv, traj_gamma,
        s_temp, knot_id
    );
...
template <typename T>
void form_schur_system_batched(
    ...
    form_schur_system_kernel<<<gridDim, blockDim, s_temp_size>>>(
        state_size, control_size, knot_points,
...
// template <typename T>
// __global__
// void form_S_gamma_Pinv_kernel_batched(
//     uint32_t solve_count,
//     uint32_t state_size,
//     uint32_t control_size,
//     uint32_t knot_points,
...
//     for (unsigned knot = blockIdx.x + blockIdx.y * gridDim.x; knot < solve_count * knot_points; knot += num_blocks) {
//         int traj_id = knot / knot_points;
//         int knot_id = knot % knot_points;
...
//         grid.sync(); //TODO: change to block.sync()?
...
//     void *kernel = (void *) form_S_gamma_Pinv_kernel_batched<T>;
//     // Get the maximum number of blocks for cooperative launch0
//     cudaDeviceProp prop;
//     int device;
...
//     int total_work_items = solve_count * knot_points;
//     int blocks = min(max_blocks/2, total_work_items);
//     printf("Blocks: %d\n", blocks);
//     dim3 grid(blocks);
//     dim3 block(SCHUR_THREADS);
//     void *args[] = {
//         (void *) &solve_count,
//         (void *) &state_size,
//         (void *) &control_size,
...
//     gpuErrchk(cudaLaunchCooperativeKernel(kernel, grid, block, args, s_temp_size));
// }
