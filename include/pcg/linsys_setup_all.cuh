...
#include "glass.cuh"
// no more matrix.cuh inclusion
// TODO: utils.cuh is from GBD_PCG include. Fix this dependency.
#include "utils.cuh"
...
template<typename T>
__device__
void complete_H_all_blockrow(uint32_t state_size, uint32_t knot_points,
...
template<typename T>
__device__
void complete_SS_Pinv_all_blockrow(uint32_t state_size, uint32_t knot_points,
    ...
    if (org_trans) {
        ...
        if (blockrow != lastrow) {
            ...
            // M3 <- M1 * M2 = theta_k_inv * phi_kp1' = Dk_inv * Ok
            __syncthreads();
            glass::gemm<T>(state_size, state_size, state_size, static_cast<T>(1.0), s_M1, s_M2, s_M3);

            // load theta_kp1_inv = Dkp1_inv to M1
            __syncthreads();
            load_block_bd<T>(state_size, knot_points,
                             d_Pinv,        // src
                             s_M1,          // dst
            ...
            // M4 <- M3 * M1 = theta_k_inv * phi_kp1' * theta_kp1_inv
        ...
        if (blockrow != 0) {
            ...
            // M3 <- M1 * M2 = theta_k_inv * phi_k = Dk_inv * Okm1'
            __syncthreads();
            glass::gemm<T>(state_size, state_size, state_size, static_cast<T>(1.0), s_M1, s_M2, s_M3);

            // load theta_km1_inv = Dkm1_inv to M1
            __syncthreads();
            load_block_bd<T>(state_size, knot_points,
                             d_Pinv,        // src
                             s_M1,          // dst
            ...
            // M4 <- M3 * M1 = theta_k_inv * phi_k * theta_km1_inv
            // M4 = Ekm1' = Dk_inv * Okm1' * Dkm1_inv
            __syncthreads();
            glass::gemm<T>(state_size, state_size, state_size, static_cast<T>(1.0), s_M3, s_M1, s_M4);
...
//      diagonal blocks of d_S, and off-diagonal blocks of d_S either partially (TRANS) or completely (ORG)
//      diagonal blocks of d_Pinv
//      all of d_gamma, d_T

// if org_trans, then everything in transformed coordinate (tilde)  -> TRANS
// if !org_trans, then everything in original coordinate            -> ORG
...
template<typename T>
__device__
void form_S_gamma_Pinv_all_blockrow(uint32_t state_size, uint32_t control_size, uint32_t knot_points,
    ...
    if (blockrow == 0) {
        ...
        __syncthreads();
        glass::addI<T>(state_size, s_M1, rho);
        ...
        // v2 <- M2 * v1 = Q0_i * q0
        __syncthreads();
        glass::gemv<T>(state_size, state_size, static_cast<T>(1.0), s_M2, s_v1, s_v2);
        ...
        if (org_trans) {
            ...
            // save M3 = T0 to d_T
            __syncthreads();
            glass::copy<T>(triangular_state, s_M3, d_T);
            ...
            // v3 <- M3 * v2 = T0 * gamma_0
            glass::trmv<T, false>(state_size, static_cast<T>(1.0), s_M3, s_v2, s_v3);
            ...
            glass::copy<T>(state_size, s_v2, d_gamma);
        }
        ...
        __syncthreads();
        glass::gemv<T>(state_size, state_size, static_cast<T>(1.0), s_M2, s_v1, s_v3);
        ...
        glass::copy<T>(control_sq, d_G + ((blockrow - 1) * G_set_size + state_sq), s_M1);
        __syncthreads();
        glass::addI<T>(control_size, s_M1, rho);
        ...
        glass::loadIdentity<T>(control_size, s_M2_tmp);

        // invert M1, M2_tmp <- inv(M1) = Rk_i
        __syncthreads();
        glass::invertMatrix<T>(control_size, s_M1, s_v1);
        ...
        glass::copy<T>(control_sq, s_M2_tmp, d_G + (blockrow - 1) * G_set_size + state_sq);
        ...
        // v2 <- M1 * v1 = -Bk * Rk_i * rk
        __syncthreads();
        glass::gemv<T>(state_size, control_size, static_cast<T>(1.0), s_M1, s_v1, s_v2);
        ...
        // v2 <- M1 * v1 = -Ak * Qk_i * qk
        __syncthreads();
        glass::gemv<T>(state_size, state_size, static_cast<T>(1.0), s_M1, s_v1, s_v2);
        ...
        if (org_trans) {
            ...
            // save phi_k' * Tk' = Okm1 * Tk' into right off-diagonal of S
            __syncthreads();
            store_block_ob<T>(state_size, knot_points,
                              s_M2,         // src
                              d_Sob,        // dst
            ...
            glass::loadIdentity<T>(state_size, s_M2);
            // M1 <- M4 = theta_k
            glass::copy<T>(state_sq, s_M4, s_M1);
            ...
            glass::copy<T>(state_size, s_v3, d_gamma + blockrow * state_size);
        }
...
template<typename T>
__global__
void form_S_gamma_Pinv_H_all_kernel(uint32_t state_size, uint32_t control_size, uint32_t knot_points,
                                    T *d_G, T *d_C, T *d_g, T *d_c,
    ...
    for (unsigned blockrow = blockIdx.x; blockrow < knot_points; blockrow += gridDim.x) {
        form_S_gamma_Pinv_all_blockrow<T>(state_size, control_size, knot_points,
                                          d_G, d_C, d_g, d_c,
    ...
    for (unsigned blockrow = blockIdx.x; blockrow < knot_points; blockrow += gridDim.x) {
        complete_SS_Pinv_all_blockrow<T>(state_size, knot_points,
                                         d_S, d_Pinv, d_H, d_T,
    ...
    if (use_H && org_trans) {
        ...
        cgrps::this_grid().sync();
        for (unsigned blockrow = blockIdx.x; blockrow < knot_points; blockrow += gridDim.x) {
            complete_H_all_blockrow<T>(state_size, knot_points,
                                       d_S, d_Pinv, d_H,
                                       s_temp, blockrow);
        }
    }
...
void transform_lamdba(uint32_t state_size, uint32_t knot_points,
    ...
    transform_lambda_kernel<<<knot_points, DZ_THREADS, s_temp_size>>>(state_size, knot_points,
                                                                      d_T, d_lambda);
}

template<typename T>
void form_schur_system_all(uint32_t state_size, uint32_t control_size, uint32_t knot_points,
    ...
    // current setup
    // form_S_gamma_Pinv_all_blockrow       4nx^2 + 3nx + 1           -> TRANS & ORG
    // complete_SS_Pinv_all_blockrow        4nx^2 + 3nx               -> TRANS
    //                                      4nx^2                     -> ORG
    // complete_H_all_blockrow              3nx^2                     -> use_H & TRANS

    // it is assumed here that nx > nu always holds
    const uint32_t s_temp_size = sizeof(T) * (4 * state_size * state_size +
                                              3 * state_size + 1);

    void *kernel = (void *) form_S_gamma_Pinv_H_all_kernel<T>;
