...
auto sqpSolvePcg(const uint32_t state_size, const uint32_t control_size,
    ...
    timespec ktt_start, ktt_end, shur_start, shur_end, dz_start, dz_end,
        ls_start, ls_end;
    ...
    auto sqpTimecheck = [&]() {
        ...
        return time_delta_us_timespec(sqp_solve_start, sqp_cur) >
               SQP_MAX_TIME_US;
    };
#else
    ...
    for (uint32_t sqpiter = 0; sqpiter < SQP_MAX_ITER; sqpiter++) {
        ...
        generate_kkt_submatrices<T>
            <<<knot_points, KKT_THREADS,
               2 * get_kkt_smem_size<T>(state_size, control_size)>>>(
                state_size, control_size, knot_points, d_G_dense, d_C_dense,
                d_g, d_c, d_dynMem_const, timestep, d_eePos_traj, d_xs, d_xu);
        gpuErrchk(cudaPeekAtLastError());
        ...
        // recover dz
        compute_dz(state_size, control_size, knot_points, d_Ginv_dense,
                   d_C_dense, d_g, d_lambda, d_dz);
        gpuErrchk(cudaPeekAtLastError());
...
#if USE_DOUBLES
        cublasDaxpy(handle, DZ_SIZE_BYTES / sizeof(T), &alphafinal, d_dz, 1,
                    d_xu, 1);
#else
        cublasSaxpy(handle, DZ_SIZE_BYTES / sizeof(T), &alphafinal, d_dz, 1,
                    d_xu, 1);
#endif
