...
int main()
{
	...
	for (uint32_t ind = 0; ind < start_goal_combinations; ind++)
	{
		...
		if (start_state == goal_state && start_state != 0)
		{
			continue;
		}
		...
		for (uint32_t pcg_exit_ind = 0; pcg_exit_ind < num_exit_vals; pcg_exit_ind++)
		{
			...
			for (uint32_t single_traj_test_iter = 0; single_traj_test_iter < traj_test_iters; single_traj_test_iter++)
			{
				...
				// Yana's
				std::cout << "goal Yana " << start_state << std::endl;
				std::cout << "printing everything that goes to MPC " << state_size << " cs " << control_size << " kp " << knot_points << std::endl;
				std::cout <<"detraj " << d_eePos_traj<< "d_xu"<<d_xu_traj<<"d_xs "<<d_xs<<" startst " <<start_state<<" goal st"<< goal_state<<std::endl;
				std::cout<<"other stuff "<<single_traj_test_iter<< "exittol "<<pcg_exit_tol<<"prefix "<< test_output_prefix << std::endl;

				//

				std::cout << "simulate MPC" << std::endl;
